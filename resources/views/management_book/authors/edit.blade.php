...
@section('content')
    <div class="max-w-4xl mx-auto mt-10 p-6 bg-white border border-gray-200 rounded-lg shadow-lg">
        ...
        <form id="edit-author-form" action="{{ route('authors.update', $author->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <textarea name="bio" id="bio" rows="4"
                    class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 @error('bio') border-red-500 @enderror">{{ old('bio', $author->bio) }}</textarea>
                @error('bio')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                <button type="button" onclick="confirmEditSubmit()"
    ...
    <script>
        function confirmEditSubmit() {
            Swal.fire({
                title: 'Yakin ingin memperbarui penulis?',
                ...
                if (result.isConfirmed) {
                    document.getElementById('edit-author-form').submit();
                }
            });
        }
    </script>
