...
<div class="max-w-6xl mx-auto mt-10 p-6 bg-white border border-gray-200 rounded-lg shadow-lg">
    ...
    <div class="mb-4">
        <a href="{{ route('books.create') }}" class="px-4 py-2 bg-amber-700 text-white rounded-md shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
            Tambah Buku Baru
        </a>
    </div>
                ...
                <tr class="border-t border-gray-200">
                    ...
                    <td class="px-4 py-2 text-sm text-gray-800">
                        @foreach($book->authors as $author)
                            <span class="inline-block px-2 py-1 text-xs bg-green-100 text-green-800 rounded-full">{{ $author->name }}</span>
                        @endforeach
                    </td>
                    <td class="px-4 py-2 text-sm text-gray-800">
                        <a href="{{ route('books.edit', $book->id) }}" class="text-indigo-600 hover:text-indigo-800">Edit</a> |
