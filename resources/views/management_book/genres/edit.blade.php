...
@section('content')
    <div class="max-w-4xl mx-auto mt-10 p-6 bg-white border border-gray-200 rounded-lg shadow-lg">
        ...
        <form id="edit-genre-form" action="{{ route('genres.update', $genre->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="4"
                    class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $genre->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                <button type="button" onclick="confirmGenreSubmit()"
                ...
                <a href="{{ route('genres.index') }}"
                    class="px-6 py-2 bg-blue-300 text-white rounded-[4px] shadow-md hover:bg-blue-500">Kembali</a>
            </div>
        </form>
    </div>
    <script>
        function confirmGenreSubmit() {
            Swal.fire({
                title: 'Yakin ingin memperbarui genre?',
                ...
                if (result.isConfirmed) {
                    document.getElementById('edit-genre-form').submit();
                }
            });
        }
    </script>
